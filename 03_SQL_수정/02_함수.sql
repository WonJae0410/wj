...
* ***********************************************/
select salary as ㅁㄴㅇㄹ from emp;
...
select emp_name, upper(emp_name) as '대문자', lower(emp_name) as '소문자', char_length(emp_name)
from emp
...
select emp_name, char_length(emp_name) -- 글자 수는 공백까지 포함
from emp
where char_length(emp_name) >= 10;


-- concat
select concat('$', salary, '달러') from emp;
select concat(emp_name, '-', dept_name) from emp; -- 같은 행의 값들을 합한다.
...
select trim('        aa     a       ');
select round(3.1234124513, 3); -- 소숫점 이하 3자리에서 반올림. (양수 ㅣ 소숫점 이하)
select round(394.1234, 0); -- 음수 : 정수부 반환
...
select emp_id, emp_name, ceil(salary * 1.15) as 'SAL_RAISE'
from emp;
...
-- 날짜 차이
select datediff(curdate(), '2025-03-24'); -- 뺀 날자 수
...
hire_date as "입사일",
adddate(hire_date, interval 6 month) as '입사 5개월 후'
from emp
...
select emp_id, emp_name, job, ifnull(dept_name, '부서미배치') as '조회'
from emp
order by 4 desc;
...
select * 
from emp
where job in ('AD_PRES', 'FI_ACCOUT', 'PU_CLERK');
...
select emp_id,
       ...
       case when salary < 5000 then salary * 0.3
            when salary between 5000 and 9999.9999 then salary * 0.2
		    else salary * 0.1 end as '급여인상'
from emp;