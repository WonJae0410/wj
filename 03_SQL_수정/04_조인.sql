...
select d.loc, e.emp_id, e.emp_name, e.hire_date
from dept d join emp e on d.dept_id = e.dept_id
...
-- emp left join dept: emp의 모든 행은 join 조건 상관없이 다 나온다.
...
select e.emp_id, e.emp_name, m.emp_name as '상사 이름', date_format(e.hire_date, '%Y/%m/%d') as 'hire date'
